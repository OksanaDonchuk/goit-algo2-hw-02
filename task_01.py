...
def optimize_printing(print_jobs: List[PrintJob], constraints: PrinterConstraints) -> Dict[str, List[str] | int]:
    """
    ...
    while jobs:
        ...
        for job in jobs[:]:  # Ітеруємо по копії списку
            if len(current_batch) < constraints.max_items and current_volume + job.volume <= constraints.max_volume:
                current_batch.append(job)
                current_volume += job.volume
                jobs.remove(job)

        # Якщо жодна модель не може поміститися, вийти з циклу
        if not current_batch:
            break

        # Додаємо ID моделей у порядок друку
        print_order.extend([job.id for job in current_batch])
        # Час друку партії – це максимальний час друку серед моделей у групі
